...
class LevelOrderIterator:
  ...
  def __init__(self, sequence):
	...
	"a perfect binary tree with lenght 2ⁿ  - 1"
      )
...
class PostOrderIterator:
  """
    ...
    iterator = PostOrderIterator(expr_tree)
    " ".join(iterator)
  """
  def __init__(self, sequence):
    if not _is_perfect_length(sequence):
  ...
  def __next__(self):
    ...
    index = self._stack.pop()
    self._index = _right_child(index)
    if self._index < len(self._sequence) and self._sequence[self._index]:
      self._node_stack.append(index)
      index = self._index
    elif self._node_stack:
      index = self._node_stack.pop()
    print(self._index, self._node_stack, index)
    result = self._sequence[index]
